...
export function ThreadList() {
  ...
  const groupedThreads = threads?.reduce(
    (acc, thread) => {
      // Format the last message date of the thread to "yyyy-MM-dd"
      const date = format(thread.lastMessageDate ?? new Date(), "yyyy-MM-dd");

      // If the date key does not exist in the accumulator, initialize it as an empty array
      if (!acc[date]) {
        acc[date] = [];
      }

      // Push the current thread into the array for the corresponding date
      acc[date].push(thread);
      return acc; // Return the updated accumulator
    },
    {} as Record<string, typeof threads>, // Initialize the accumulator as an empty object
  );
